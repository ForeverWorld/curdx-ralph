...
- `/curdx:start` -> Auto-detect: resume active or ask for new
- `/curdx:start user-auth` -> Resume or create user-auth
- `/curdx:start user-auth Add OAuth2` -> Create user-auth with goal
- `/curdx:start user-auth --fresh` -> Force new, overwrite if exists
- `/curdx:start "Build auth with JWT" --quick` -> Quick mode with goal string
- `/curdx:start my-feature "Add logging" --quick` -> Quick mode with name+goal
- `/curdx:start ./my-plan.md --quick` -> Quick mode with file input
- `/curdx:start my-feature ./plan.md --quick` -> Quick mode with name+file
- `/curdx:start my-feature --quick` -> Quick mode using existing plan.md
- `/curdx:start my-feature "Add logging" --tasks-size coarse` -> Coarse granularity (10-20 tasks)
- `/curdx:start my-feature --quick --tasks-size fine` -> Quick mode with fine granularity
...
2. ONE ARG before --quick:
   ...
   b. KEBAB-CASE NAME: matches ^[a-z0-9-]+$
      ...
      - If found but no plan.md: error "No plan.md found in $specPath/. Provide goal: /curdx:start $name 'your goal' --quick"
      - If not found: error "Spec '$name' not found. Provide goal: /curdx:start $name 'your goal' --quick"
...
2. If found: Check if `$specPath/plan.md` exists
   ...
   - If plan.md not exists: error "No plan.md found in $specPath. Provide goal: /curdx:start $name 'your goal' --quick"
3. If not found: error "Spec '$name' not found. Provide goal: /curdx:start $name 'your goal' --quick"

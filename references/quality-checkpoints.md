...
### Control Panel Verification (Conditional)

For each control-panel target from research.md:

1. `CP1` start dependencies and wait for panel URL
2. `CP2` login/configure via MCP `chrome-devtools`
3. `CP3` verify config via API/CLI (not UI-only)
4. `CP4` cleanup processes/resources

...
### Historical Research (if claude-mem available)

Before dispatching the research team, try to retrieve past research.
These MCP tools may or may not exist — if they error, skip silently and proceed.

- If MCP tool `search` is available: query for similar past research findings, discoveries, or technical evaluations related to this spec's goal.
- Use results as additional context for the research team's prompts (include as "Prior Knowledge" section).
...
1. Display: `-> Next: Run /curdx:requirements`
2. End your response immediately
3. Wait for user to explicitly run `/curdx:requirements`
</mandatory>

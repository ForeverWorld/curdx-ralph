...
4. Check `design.md` exists. If not, error: "Design not found. Run /curdx:design first."
...
7. **`--tasks-size` flag handling**: Check `$ARGUMENTS` for `--tasks-size` flag:
   - If value is `fine` or `coarse`: update `granularity` in `.curdx-state.json` to the given value (overrides any value set by `/curdx:start`)
   ...
   - If `--tasks-size` flag is absent: leave `granularity` unchanged in `.curdx-state.json` (preserve any value set by `/curdx:start`)
...
- `granularity` is NOT already set in `.curdx-state.json` (i.e., not pre-set via `--tasks-size` flag on `/curdx:start` or `/curdx:tasks`)
...
1. Display: `-> Next: Run /curdx:implement to start execution`
2. End your response immediately
3. Wait for user to explicitly run `/curdx:implement`
</mandatory>

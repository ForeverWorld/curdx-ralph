...
Examples:
- `/curdx:new user-auth` -> name="user-auth", goal=none
- `/curdx:new user-auth Add OAuth2 login` -> name="user-auth", goal="Add OAuth2 login"
- `/curdx:new user-auth --skip-research` -> name="user-auth", goal=none, skip research
- `/curdx:new api-auth --specs-dir ./packages/api/specs` -> create in specified dir
...
3. Only proceed to requirements when user explicitly runs `/curdx:requirements`
...
- Run /curdx:requirements to proceed (or /curdx:design if skipped research)
```

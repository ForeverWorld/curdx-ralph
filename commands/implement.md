...
**Configuration**: Specs directories are configured in `.claude/curdx.local.md`:
```yaml
...
3. If no active spec, error: "No active spec. Run /curdx:new <name> first."

**Validate**:
1. Check the resolved spec directory exists
2. Check the spec's tasks.md exists. If not: error "Tasks not found. Run /curdx:tasks first."
...
- Missing/corrupt state file: error and suggest re-running /curdx:implement
- Missing tasks.md: error and suggest running /curdx:tasks
- Missing spec directory: error and suggest running /curdx:new
...
5. Update spec index: `${CLAUDE_PLUGIN_ROOT}/hooks/scripts/update-spec-index.sh --quiet`

...
Smart entry point for curdx. Detects whether to create a new spec or resume an existing one.
...
### Historical Context (if claude-mem available)

Before parsing, try to retrieve relevant historical context.
These MCP tools may or may not exist — if they error, skip silently and proceed.

- If MCP tool `search` is available: call it with the task name/goal description to find relevant past specs, decisions, or patterns from previous sessions.
- If not available: proceed without — this is fine. claude-mem integration is optional.
...
- If user accepts: invoke `/curdx:triage` with no positional args and let triage collect epic-name + goal interactively. STOP.
- If user declines: continue normal Step 4 routing.
...
9. **Skill Discovery Pass 1** -- Scan all skill files and match against the goal text:
   1. Scan SKILL.md files from all skill paths (collect all skills before matching):
      - **Plugin skills**: `${CLAUDE_PLUGIN_ROOT}/skills/*/SKILL.md` → invoked as `Skill({ skill: "curdx:<name>" })`
...
10. Update Spec Index: `${CLAUDE_PLUGIN_ROOT}/hooks/scripts/update-spec-index.sh --quiet`
...
Then STOP. Output: `-> Next: Run /curdx:requirements`

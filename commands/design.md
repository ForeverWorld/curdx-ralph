...
2. If no active spec, error: "No active spec. Run /curdx:new <name> first."
3. Check the resolved spec directory exists
4. Check `requirements.md` exists. If not, error: "Requirements not found. Run /curdx:requirements first."
...
### Historical Decisions (if claude-mem available)

Before dispatching the architect, try to retrieve past architectural decisions.
These MCP tools may or may not exist — if they error, skip silently and proceed.

- If MCP tool `search` is available: query for past architectural decisions, design patterns, and technology choices related to this area.
- Use results as additional context for the architect-reviewer's prompt (include as "Historical Decisions" section).
...
1. Display: `-> Next: Run /curdx:tasks`
2. End your response immediately
3. Wait for user to explicitly run `/curdx:tasks`
</mandatory>

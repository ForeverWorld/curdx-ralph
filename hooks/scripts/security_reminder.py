...
def main():
    ...
    with HookTimer("security_reminder", "PreToolUse") as t:
        ...
        if security_reminder_enabled == "0":
            t.set(message="disabled via ENABLE_SECURITY_REMINDER=0", decision="skip")
            sys.exit(0)
        ...
        if tool_name not in ["Edit", "Write", "MultiEdit"]:
            t.set(message=f"irrelevant tool, skipping", decision="allow")
            sys.exit(0)  # Allow non-file tools to proceed

        # Extract file path from tool_input
        file_path = tool_input.get("file_path", "")
        if not file_path:
            t.set(message="no file_path in input", decision="allow")
            sys.exit(0)  # Allow if no file path
        t.set(file=file_path)
        ...
        if rule_name and reminder:
            ...
            if warning_key not in shown_warnings:
                ...
                t.set(message=f"matched rule: {rule_name}", decision="deny", extra={"rule": rule_name})
                sys.exit(2)  # Block tool execution (exit code 2 for PreToolUse hooks)
            else:
                t.set(message=f"rule {rule_name} already shown this session", decision="allow")
        else:
            t.set(message="no security patterns matched", decision="allow")

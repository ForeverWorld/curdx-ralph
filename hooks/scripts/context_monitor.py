...
def _get_session_id() -> str:
    """Get CURDX session ID from environment."""
    return os.environ.get("CURDX_SESSION_ID", "").strip() or "default"
...
def _read_statusline_context_pct() -> float | None:
    ...
    Cache is already scoped per CURDX session via CURDX_SESSION_ID.
    """
    pilot_session_id = os.environ.get("CURDX_SESSION_ID", "").strip()
    if not pilot_session_id:
        return None
    cache_file = Path.home() / ".curdx" / "sessions" / pilot_session_id / "context-pct.json"
...
def _resolve_context(session_id: str) -> tuple[float, int, list[int], bool] | None:
    ...
    written by the statusline process for this specific CURDX session.
    """
...
def run_context_monitor() -> int:
    ...
    with HookTimer("context_monitor", "PostToolUse") as t:
        ...
        if resolved is None:
            t.set(message="no context data available (statusline cache missing)", decision="skip")
            return 0

        percentage, total_tokens, shown_learn, shown_80_warn = resolved
        effective = _to_effective(percentage)
        t.set(extra={"raw_pct": f"{percentage:.1f}", "effective_pct": f"{effective:.0f}", "tokens": str(total_tokens)})
        ...
        if percentage < THRESHOLD_AUTOCOMPACT:
            for threshold in LEARN_THRESHOLDS:
                if percentage >= threshold and threshold not in shown_learn:
                    ...
                    t.set(message=f"learn prompt at {effective:.0f}% (threshold {threshold})", decision="context")
                    break
        ...
        if percentage >= THRESHOLD_AUTOCOMPACT:
            ...
            t.set(message=f"auto-compact warning at {effective:.0f}%", decision="context")
            return 0
        ...
        if percentage >= THRESHOLD_WARN and not shown_80_warn:
            ...
            t.set(message=f"first warning at {effective:.0f}%", decision="context")
            return 0

        if percentage >= THRESHOLD_WARN and shown_80_warn:
            if new_learn_shown:
                save_cache(total_tokens, session_id, new_learn_shown)
            t.set(message=f"above warn threshold {effective:.0f}%, already warned", decision="allow")
            return 0

        if new_learn_shown:
            save_cache(total_tokens, session_id, new_learn_shown)

        if not new_learn_shown:
            t.set(message=f"context OK at {effective:.0f}%", decision="allow")

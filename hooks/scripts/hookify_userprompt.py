...
def main():
    ...
    with HookTimer("hookify_userprompt", "UserPromptSubmit") as t:
        try:
            ...
            if result:
                t.set(message=f"prompt rules evaluated, result has keys: {list(result.keys())}", decision="applied")
            else:
                t.set(message="no prompt rules matched", decision="allow")

...
def main():
    ...
    with HookTimer("hookify_posttooluse", "PostToolUse") as t:
        try:
            ...
            if result:
                t.set(message=f"rules evaluated, result has keys: {list(result.keys())}", decision="applied")
            else:
                t.set(message="no rules matched", decision="allow")

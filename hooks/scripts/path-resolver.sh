#!/bin/bash
# Path Resolution Helper for CURDX Specum
...
CURDX_SETTINGS_FILE="$RALPH_CWD/.claude/curdx.local.md"
...
_curdx_validate_cwd() {
    if [ ! -d "$RALPH_CWD" ]; then
        _curdx_warn "RALPH_CWD does not exist: $RALPH_CWD"
        return 1
    fi
    return 0
}
    ...
    while IFS= read -r dir; do
        ...
        if [[ "$dir" == /* ]] && [[ "$dir" != "$RALPH_CWD"* ]]; then
            ...
            if [ ! -d "$RALPH_CWD/$dir" ]; then
                _curdx_warn "Skipping invalid path in specs_dirs: $dir (directory not found at $RALPH_CWD/$dir)"
                continue
            fi
        fi
...
curdx_find_spec() {
    ...
    if [ $count -eq 0 ]; then
        ...
        echo "$found" | nl >&2
        echo "Specify full path: /curdx:switch <path>" >&2
        return 2
    fi
}
